...
- **commerce 계약(`commerce-`)은 읽기 전용으로 교차참조**만 합니다 — 절대 수정하지 않아 소비 제품의 parity baseline을 깨지 않습니다.

```
commerce- (commerce baseline) ──(read-only cross-ref)──▶ presets/experimental-rules/fields 검증
distribution-
  base.json            # 유통 통제 어휘
