...
test("skills validator rejects proxy route mismatches", (t) => {
  const fixtureRoot = createContractsFixture(t);
  const catalog = readFixtureJson(fixtureRoot, "contracts/skills/v1/catalog.json");
  ...
  writeFixtureJson(fixtureRoot, "contracts/skills/v1/catalog.json", catalog);
...
test("benchmarks validator rejects results for unknown targets", (t) => {
  const fixtureRoot = createContractsFixture(t);
  const results = readFixtureJson(fixtureRoot, "contracts/benchmarks/v1/results.json");
  results.results[0].targetId = "missing-target-for-test";
  writeFixtureJson(fixtureRoot, "contracts/benchmarks/v1/results.json", results);
