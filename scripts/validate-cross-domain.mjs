...
const platformDocs = readContract("platform-frontmatter", "documents");
const platformStatusByObj = new Map(platformDocs.map((d) => [d.documentType, d.statuses ?? []]));
const commerceStatuses = readDoc("commerce-base").core?.statuses ?? {};
...
const platformGroups = new Map((readContract("platform-event-types", "groups")).map((g) => [g.object, g.events]));
const commerceEventsByObj = readDoc("commerce-base").core?.eventTypesByObject ?? {};
...
const gameopsBase = readDoc("gameops-base");
...
const benchStatuses = new Set(readDoc("benchmarks-base").modelStatuses ?? []);
const routing = readDoc("governance-model-routing").riskModelStatus ?? [];
...
const govRisk = new Set(readDoc("governance-base").riskLevels ?? []);
const govPolicies = new Set(readDoc("governance-base").approvalPolicies ?? []);
for (const pb of readContract("gameops-playbooks", "playbooks")) {
