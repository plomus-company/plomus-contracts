# Distribution Contracts (`contracts/distribution/v1/`)
...
- **commerce 계약(`contracts/commerce/v1/`)은 읽기 전용으로 교차참조**만 합니다 — 절대 수정하지 않아 소비 제품의 parity baseline을 깨지 않습니다.

```
contracts/commerce/v1/ (commerce baseline) ──(read-only cross-ref)──▶ presets/experimental-rules/fields 검증
